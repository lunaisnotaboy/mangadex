...
namespace :repo do
  ...
  task :authors do
    ...
    url = 'https://api.github.com/repos/lunaisnotaboy/mangadex/contributors?anon=1'
        ...
        file.each_line do |line|
          if line.start_with?('-')
            new_line = line.gsub(/#([[:digit:]]+)*/) do |pull_request_reference|
              ...
              loop do
                response = HTTP.headers('Authorization' => "token #{ENV['GITHUB_API_TOKEN']}").get("https://api.github.com/repos/lunaisnotaboy/mangadex/pulls/#{pull_request_number}")
